 ...
 */
    ...
    */
    computeAABB(){
        ...
        // record the current min and max XYZ
        var currMinXYZ = [1,1,1];
        var currMaxXYZ = [-1,-1,-1];

        // traverse the whole vertices buffer
        for(var i = 0; i < this.numVertices; i++){

            // get the current XYZ
            var currXYZ =[this.vBuffer[i*3], this.vBuffer[i*3+1], this.vBuffer[i*3+2]];
            
            // cmp all three dimensions
            for(var j = 0; j < 3; j++){
                if(currXYZ[j] < currMinXYZ[j]){
                    currMinXYZ[j] = currXYZ[j];
                }
                if(currXYZ[j] > currMaxXYZ[j]){
                    currMaxXYZ[j] = currXYZ[j];
                }
            }
        }

        // geting the final result
        this.minXYZ = currMinXYZ;
        this.maxXYZ = currMaxXYZ;
    ...
    */
    loadFromOBJ(fileText)
    {
        ...
        for(var i = 0; i < lines.length; i++){
            ...
            // put vertices into vBuffer
            if(datas[0] == 'v'){
                this.vBuffer.push(parseFloat(datas[1]));
                this.vBuffer.push(parseFloat(datas[2]));
                this.vBuffer.push(parseFloat(datas[3]));
                this.numVertices ++;
            }

            // put faces into fBuffer
            if(datas[0] == 'f'){
                this.fBuffer.push(parseInt(datas[2])-1);
                this.fBuffer.push(parseInt(datas[3])-1);
                this.fBuffer.push(parseInt(datas[4])-1);
                this.numFaces ++;
            }
