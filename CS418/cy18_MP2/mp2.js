...
/** @global Parameter used to control move-speed */
var speed = 0.001;
/** @global Parameter used in rotation */
var quaternion = quat.create();
/** @global Parameter descirbing x-aix of view plane */
...
/**@global dict to record pressed key*/
...
function setupShaders() {
  ...
  shaderProgram.uniformIsFogOpen = gl.getUniformLocation(shaderProgram, "uIsFogOpen");
}
...
function draw() { 
    ...
    // vec3.set(eyePt, 0.2, 0.5, -0.7);
    // vec3.set(viewDir, -0.15, -0.6, -1.0);
    ...
    if(document.getElementById("fogOpen").checked){
      gl.uniform1f(shaderProgram.uniformIsFogOpen, 1.0);
    }else{
      gl.uniform1f(shaderProgram.uniformIsFogOpen, 0.0);
    }
...
function animate(){
  ...
  if(currentlyPressedKeys["ArrowUp"]){
    // get quaternion about x-axis
    quat.setAxisAngle(quaternion, x_axis, degToRad(0.2));
    // reset viewDir-vector and up-vector
  ...
  if(currentlyPressedKeys["ArrowLeft"]){
    // get quaternion about viewDir
    quat.setAxisAngle(quaternion, viewDir, degToRad(-0.3));
    // reset up-vector and x_axis
    vec3.transformQuat(up, up, quaternion);
    vec3.transformQuat(x_axis, x_axis, quaternion);
  }
  ...
  if(currentlyPressedKeys["a"]){
    // get quaternion about up-vector
    quat.setAxisAngle(quaternion, up, degToRad(0.15));
    // reset up-vector and view direction
    vec3.transformQuat(viewDir, viewDir, quaternion);
    vec3.transformQuat(x_axis, x_axis, quaternion);
  }
